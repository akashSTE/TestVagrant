...
import static Core.GlobalData.CITY_TO_PINNED_AND_VERIFIED;
import static Core.GlobalData.CITY_WEATHER_DETAILS_ON_MAP;
...
public class Phase1 extends BaseTest {
    ...
    public void pinCityOnMap(){
        Assert.assertTrue(weatherPage.selectCity(CITY_TO_PINNED_AND_VERIFIED));
    }

    @Test(priority = 4)
    public void cityOnMapTempInCel(){
        Assert.assertTrue(weatherPage.isCityOnMapTempInCelDisplayed(CITY_TO_PINNED_AND_VERIFIED));
    }

    @Test(priority = 5)
    public void cityOnMapTempInFeh(){
        Assert.assertTrue(weatherPage.isCityOnMapTempInFehDisplayed(CITY_TO_PINNED_AND_VERIFIED));
    }
